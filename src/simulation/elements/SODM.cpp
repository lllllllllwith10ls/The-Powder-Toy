		...
		for (int ry = -2; ry <= 2; ++ry)
		{
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				case PT_CBNW:
					...
					else if(RNG::Ref().chance(1, oxygenationFactor*20 + 1))
					{
				...
				case PT_FIRE:
					if (self.temp > 350.f && RNG::Ref().chance(1, 40) && hydrogenationFactor < 20)
					{
						burnTimer = 13;
						hydrogenationFactor += 1;
					}
				...
				case PT_O2:
					if (burnTimer > 0 && RNG::Ref().chance(1, 10))
					{
...
static int graphics(GRAPHICS_FUNC_ARGS)
{
	...
	// Oxygenated sodm
		...
		*colr += (s*(PIXR(colour) - *colr))/25;
		*colg += (s*(PIXG(colour) - *colg))/25;
		*colb += (s*(PIXB(colour) - *colb))/25;
	}
	return 0;
}